...
//                PlayerColor.BLUE, "Bernard");
        ...
        // add occupant
        var state = initialGameState(List.of(61, 1, 62, 0), List.of());

        var placedTile61 = new PlacedTile(state.tileToPlace(), PlayerColor.RED, Rotation.NONE, new Pos(0, -1));
        var state1 = state.withPlacedTile(placedTile61).withNewOccupant(new Occupant(Occupant.Kind.PAWN, 61_0));

//        var placedTile1 = new PlacedTile(state.tileToPlace(), PlayerColor.RED, Rotation.NONE, new Pos(0, -2));
//        var state2 = state1
//                .withPlacedTile(placedTile1).withNewOccupant(new Occupant(Occupant.Kind.PAWN, 1_3));

        var gameStateO = new SimpleObjectProperty<>(gameState);
        var gameState1 = new SimpleObjectProperty<>(state1);
        //var gameState2 = new SimpleObjectProperty<>(state2);

        var playersNode = PlayersUI.create(gameState1, textMaker);
